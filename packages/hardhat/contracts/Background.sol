...
import "./Libraries/BackgroundLibrary.sol";
...
contract Background is ERC721Enumerable {
    ...
    mapping(uint256 => uint256) public socks_backgrounds;

    //! Properties types
    string[11] public backgrounds;

    constructor() ERC721("SocksBackground", "SKBG") {
        backgrounds = [
            "tiles",
            "wall",
            "wood",
    ...
    function mintItem() public payable returns (uint256) {
        ...
        socks_backgrounds[id] = uint256(((uint8(predictableRandom[12])) % 10));
    ...
    function renderTokenById(uint256 id) public view returns (string memory) {
        uint256 bgIndex = getPropertiesById(id) + 1;

        string memory render = BackgroundLibrary.GetBackground(bgIndex);
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        string memory name = string(
            abi.encodePacked("Sock Background #", id.toString())
        );

        string memory description = string(
            abi.encodePacked("Background: ", backgrounds[bgIndex])
        );
