...
import "./Libraries/PatternLibrary.sol";
import "hardhat/console.sol";

// GET LISTED ON OPENSEA: https://testnets.opensea.io/get-listed/step-two

contract Pattern is ERC721Enumerable {
    ...
    mapping(uint256 => uint256) public socks_patterns;

    //! Properties types
    string[15] public patterns;

    constructor() ERC721("SocksPattern", "SKPTRN") {
        patterns = [
            "lightening",
            "line",
            "saturn",
    ...
    function mintItem() public payable returns (uint256) {
        ...
        socks_patterns[id] = uint256((uint8(predictableRandom[6])) % 15);
    ...
    function renderTokenById(uint256 id) public view returns (string memory) {
        uint256 pattern = getPropertiesById(id) + 1;

        string memory render = string(
            abi.encodePacked(
                '<g class="cls-1">',
                '<g id="Pattern">',
                PatternLibrary.GetPattern(pattern),
                "</g>",
                "</g>"
            )
        );
    ...
    function getDescription(uint256 id) public view returns (string memory) {
        ...
        uint256 pattern = getPropertiesById(id);
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        string memory name = string(
            abi.encodePacked("Sock Pattern #", id.toString())
        );

        string memory description = string(
            abi.encodePacked("Pattern: ", patterns[pattern])
        );
    ...
    function getPropertiesById(uint256 id)
        public
        view
        returns (uint256 pattern)
    {
        pattern = socks_patterns[id];
    }
}
