...
import "./Libraries/LegLibrary.sol";
import "hardhat/console.sol";

// GET LISTED ON OPENSEA: https://testnets.opensea.io/get-listed/step-two

contract Leg is ERC721Enumerable {
    ...
    uint256 public constant curve = 1001; // price increase 0,5% with each purchase
    uint256 public price = 0.001 ether;

    mapping(uint256 => uint256) public socks_leg;

    //! Properties types
    string[3] public colors;

    constructor() ERC721("SocksLeg", "SKLEG") {
        colors = ["#FCDFB4", "#D2B17F", "#FAF3B3"];
    }
    ...
    function mintItem() public payable returns (uint256) {
        ...
        socks_leg[id] = uint256((uint8(predictableRandom[8])) % 3);
    ...
    function renderTokenById(uint256 id) public view returns (string memory) {
        ...
        string memory render = string(
            abi.encodePacked(LegLibrary.GetLeg(colors[color]))
        );
    ...
    function getDescription(uint256 id) public view returns (string memory) {
        ...
        uint256 leg = getPropertiesById(id);
        string memory desc = string(abi.encodePacked("Color: ", colors[leg]));
        return desc;
    }
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        ...
        string memory name = string(
            abi.encodePacked("Sock Leg #", id.toString())
        );

        string memory description = string(
            abi.encodePacked("Color: ", colors[leg])
        );
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                id.toString(),
                                '","attributes":[{"trait_type":"LegColor","value":"',
                                colors[leg],
                                '"}], "owner":"',
    ...
    function getPropertiesById(uint256 id) public view returns (uint256 leg) {
        leg = socks_leg[id];
    }
}
